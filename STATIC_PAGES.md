...
- **戸建**: 19,529ページ（町名・大字レベル）
- **土地**: 19,529ページ（町名・大字レベル）
- **合計**: 39,058ページ
- **サイズ**: 2.5GB
...
# または
npm run generate:static:by-pref
```
...
```bash
# 大阪府のみ
node scripts/generate_static_incremental.mjs 大阪府 all

# 滋賀県の戸建のみ
node scripts/generate_static_incremental.mjs 滋賀県 house

# 京都府の土地のみ
node scripts/generate_static_incremental.mjs 京都府 land
```

### 一括生成（非推奨：メモリ大量使用）

```bash
# 全ページ一度に生成（5〜10分、メモリ2GB以上必要）
