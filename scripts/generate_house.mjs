...
let outputPath;
if (mode === 'area') {
  outputPath = path.join(PROJECT_ROOT, 'data', 'house', 'area', sanitize(pref), `${sanitize(city)}.json`);
} else {
  outputPath = path.join(PROJECT_ROOT, 'data', 'house', 'station', sanitize(rail), sanitize(line), `${sanitize(station)}.json`);
}
...
if (mode === 'area') {
  templatePath = path.join(__dirname, 'templates', 'area_prompt.txt');
  ...
  templatePath = path.join(__dirname, 'templates', 'station_prompt.txt');
  const template = fs.readFileSync(templatePath, 'utf-8');
...
function parseClaudeOutput(output) {
  const match = output.match(/const\s+kodate_db\s*=\s*(\[[\s\S]*?\]);/);
  if (!match) {
    throw new Error('const kodate_db = [...]; の形式が見つかりません');
  }
  const jsonStr = match[1];
  return JSON.parse(jsonStr);
}

// 検証関数（saiseisei3スキーマ対応版）
function validateData(data, mode, params) {
  ...
  const requiredKeys = [
    ...
    'landArea', 'buildingArea', 'layout', 'buildingAge'
  ];
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    if (item.type !== '戸建て') {
      return { valid: false, error: `レコード${i + 1}: type="${item.type}" (正: "戸建て")` };
    }
